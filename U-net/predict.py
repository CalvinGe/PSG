...
def predict(model_path, spec_path, channel_idx, label_idx, path1, path2, test_path):
    ...
    for filename in all_test_files:
        ...
        for i in [1, 2, 3, 4, 5]:
            ...
            try:
                _, sensitivity, specificity, precision, accuracy, F1, auprc_baseline = evaluations_matrix_binary(gs_i,
                                                                                                                 pred_bi)
                all_test_eva.write('%s\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%d\n' % (
                the_id, labels[i], auroc, auprc, sensitivity, specificity, precision, accuracy, F1, auprc_baseline,
                len(idx)))
        ...
        try:
            conf_mat, sensitivity, specificity, precision, accuracy, F1, auprc_baseline = evaluations_matrix_binary(
                gs_i, pred_bi)
            all_test_eva.write('%s\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%d\n' % (
            the_id, 'Arousal', auroc, auprc, sensitivity, specificity, precision, accuracy, F1, auprc_baseline,
            len(idx)))
            print('%s\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%d\n' % (
            the_id, 'Arousal', auroc, auprc, sensitivity, specificity, precision, accuracy, F1, auprc_baseline,
            len(idx)))
...
def main():
    ...
    parser.add_argument('-c', '--channel_idx', type=int,
                        nargs='+',
                        default=[2, 3, 5, 4, 6, 7, 0, 11],
                        help=''' a list, index of selected signals. 2: F3-M2, 3: F4-M1, 5:C3-M2, 4:C4-M1, 6:O1-M2 , 7:O2-M1, 0: E1-M2, 11: ECG.
            default: all ''')
    ...
    parser.add_argument('--sig_path', type=str, default='./data/avg16_16m_multi_task_image/',
                        help='signal path, default = ./data/avg16_16m_multi_task_image/')

    parser.add_argument('--lab_path', type=str, default='./data/avg16_16m_multi_task_label/',
                        help='label path, default = ./data/avg16_16m_multi_task_label/')

    parser.add_argument('--test_path', type=str, default='whole_test.txt',
                        help='test path, default = whole_test.txt')
...
def run(channel_idx, label_idx, sig_path, lab_path, test_path):
    model_path = 'D:/学习资料/PSG/CopyGuan/weights/'
